CREATE OR REPLACE FUNCTION fn_test(pv_producto VARCHAR) 
RETURNS TABLE(tv_title VARCHAR) AS
$$
BEGIN
RETURN QUERY SELECT title
FROM products 
WHERE title ILIKE '%'||pv_producto||'%';
END;
$$
LANGUAGE PLPGSQL
...
$$
BEGIN
RETURN QUERY SELECT prod_id, title, price
FROM products
